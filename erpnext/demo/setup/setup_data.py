...
def setup_user_roles(domain):
	user = frappe.get_doc('User', 'demo@ektai.io')
	...
	if not frappe.db.get_global('demo_hr_user'):
		user = frappe.get_doc('User', 'CaitlinSnow@ektai.mail')
		...
		for d in frappe.get_all('User Permission', filters={"user": "CaitlinSnow@ektai.mail"}):
			frappe.delete_doc('User Permission', d.name)

	if not frappe.db.get_global('demo_sales_user_1'):
		user = frappe.get_doc('User', 'VandalSavage@ektai.mail')
	...
	if not frappe.db.get_global('demo_sales_user_2'):
		user = frappe.get_doc('User', 'GraceChoi@ektai.mail')
	...
	if not frappe.db.get_global('demo_purchase_user'):
		user = frappe.get_doc('User', 'MaxwellLord@ektai.mail')
	...
	if not frappe.db.get_global('demo_manufacturing_user'):
		user = frappe.get_doc('User', 'NeptuniaAquaria@ektai.mail')
	...
	if not frappe.db.get_global('demo_stock_user'):
		user = frappe.get_doc('User', 'HollyGranger@ektai.mail')
	...
	if not frappe.db.get_global('demo_accounts_user'):
		user = frappe.get_doc('User', 'BarryAllen@ektai.mail')
	...
	if not frappe.db.get_global('demo_projects_user'):
		user = frappe.get_doc('User', 'PeterParker@ektai.mail')
	...
	user = frappe.get_doc('User', 'ClarkKent@ektai.mail')
